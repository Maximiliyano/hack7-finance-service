<template>
  <v-container fluid>
    ...
    >
      <v-card>
        <v-card-title>
          <span class="text-h5">New Credit</span>
        </v-card-title>
    ...
    <v-row>
      <v-col cols="12" md="6" lg="4" v-for="(credit, index) in user.credits" v-bind:key="credit.id">
    <v-card
      class="mx-auto"
    >
      ...
      <v-expand-transition>
        <div v-show="show === index">
          ...
          <v-card-text>
            ...
            <v-row justify="center" class="my-0">
              <v-col cols="5" class="text-left px-0" style="padding-right: 5px;">Sum by payments:</v-col>
              <v-col cols="5" class="error--text text-right px-0" style="padding-left: 5px;">{{ (calculateCreditPayment(credit)*(getTerm(credit)))}}$</v-col>
            </v-row>
            <v-row justify="center" class="my-0">
...
export default {
  ...
  methods: {
    ...
    calculateCreditPayment (credit) {
      ...
      const monthlyPayment = initialSum * monthlyInterestRate + initialSum / monthTerm
